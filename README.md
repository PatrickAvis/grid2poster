...
| `--cable-sea-buffer-km` | region preset (`600` UK) | Inflate the land boundary over water for submarine cable queries (when `--include-cables` is on) and as the default offshore extent for plant/turbine exports. Set to `0` to disable cables' sea margin. |
| `--offshore-sea-buffer-km` | `250` UK (less than cables) | Sea buffer for `power=plant` and wind turbine Overpass queries. Smaller than the cable margin so tile counts stay practical; enough for North Sea wind farms. |
...
| `uk_etys_boundaries_web.geojson` | NESO ETYS transmission boundaries | WGS84 lines (B6, B9, B2, …); constraint boundaries, not tariff zones |
...
DNO, GSP, and TNUoS generation charging zone polygons are not in OSM. One script fetches every registered NESO boundary dataset (GeoJSON, ZIP, or GPKG sources) into `data/zones/` as normalized WGS84 GeoJSON:

```powershell
python scripts/fetch_neso_zones.py
python scripts/fetch_neso_zones.py --only generation
python scripts/fetch_neso_zones.py --list
python scripts/prepare_map_data.py --region uk --skip-lines --skip-plants --skip-substations --skip-turbines
```

Add future NESO overlays (SSEP, tRESP, ETYS, etc.) by registering a new entry in `NESO_DATASETS` inside `scripts/fetch_neso_zones.py` — no separate fetch script required.
...
| [NESO — GB generation charging zones](https://www.neso.energy/data-portal/gis-boundaries-gb-generation-charging-zones) | `data/zones/uk_generation_charging_zones.geojson` | Toggle **TNUoS generation zones**; click to filter |
| [NESO — ETYS GB transmission system boundaries](https://www.neso.energy/data-portal/etys-gb-transmission-system-boundaries) | `data/zones/uk_etys_boundaries.geojson` | Toggle **ETYS transmission boundaries** (B6, B9, …) |

Generation charging zone boundaries are approximate and can change over time; treat them as indicative on the map. ETYS boundaries are transmission constraint lines from the [Electricity Ten Year Statement](https://www.neso.energy/publications/electricity-ten-year-statement-etys/electricity-transmission-network-requirements), not settlement areas.
...
- DNO/GSP/TNUoS generation zone boundaries, if used separately, should follow [NESO](https://www.neso.energy/data-portal) or the relevant DNO open-data licence.
