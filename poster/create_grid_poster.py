...
from poster.render import load_logo_image, render_poster
from poster.theming import list_themes, load_theme
from prepare import prepare_lines, prepare_plants, prepare_wind_turbines
...
def parse_args(argv: Iterable[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--theme", "-t", default="paper_grid", help="Theme ID from poster/themes/")
    ...
    parser.add_argument(
        ...
        const="",
        default=None,
        help="Also save all transmission lines as CSV with WGS84 geometry_wkt. "
             "Optionally pass a path; otherwise written next to the poster.",
    )
    parser.add_argument(
        "--export-substations-geojson",
        nargs="?",
        const="",
        default=None,
        help="Also save OSM power=substation features as a GeoJSON (WGS84). "
             "Optionally pass a path; otherwise written next to the poster.",
    )
    parser.add_argument(
        "--export-substations-csv",
        nargs="?",
        const="",
        default=None,
        help="Also save OSM power=substation features as CSV with WGS84 geometry_wkt. "
             "Optionally pass a path; otherwise written next to the poster.",
    )
    parser.add_argument(
        "--export-plants-geojson",
        nargs="?",
        const="",
        default=None,
        help="Also save OSM power=plant features as a GeoJSON (WGS84). "
             "Optionally pass a path; otherwise written next to the poster.",
    )
    parser.add_argument(
        "--export-plants-csv",
        ...
        const="",
        default=None,
        help="Also save OSM power=plant features as CSV with WGS84 geometry_wkt. "
             "Optionally pass a path; otherwise written next to the poster.",
    )
    parser.add_argument(
        "--export-turbines-geojson",
        nargs="?",
        const="",
        default=None,
        help="Also save OSM wind turbines (power=generator) as a GeoJSON (WGS84). "
             "Optionally pass a path; otherwise written next to the poster.",
    )
    parser.add_argument(
        "--export-turbines-csv",
        ...
        const="",
        default=None,
        help="Also save OSM wind turbines (power=generator) as CSV with WGS84 geometry_wkt. "
             "Optionally pass a path; otherwise written next to the poster.",
    )
    parser.add_argument(
        "--single-query",
...
def main(argv: Iterable[str] = sys.argv[1:]) -> int:
    ...
    raw_plants = None
    if args.show_plants or args.export_plants_geojson is not None or args.export_plants_csv is not None:
    ...
    if args.export_geojson is not None:
        ...
        save_geojson(export, geojson_path)
    ...
    if args.export_csv is not None:
        if args.export_csv:
            ...
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            csv_path = POSTERS_DIR / f"{slugify(args.country)}_grid_{timestamp}.csv"
        export = lines_projected.to_crs("EPSG:4326").drop(columns=["sort_voltage"], errors="ignore")
        save_csv(export, csv_path)
        print(f"Saved CSV: {csv_path}")

    if args.export_plants_geojson is not None:
        if raw_plants is None:
            raise RuntimeError("Power plants were not fetched before export")
        if args.export_plants_geojson:
            plants_geojson_path = Path(args.export_plants_geojson)
        else:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            plants_geojson_path = POSTERS_DIR / f"{slugify(args.country)}_plants_{timestamp}.geojson"
        save_geojson(raw_plants, plants_geojson_path)
        print(f"Saved plants GeoJSON: {plants_geojson_path}")

    if args.export_plants_csv is not None:
        if raw_plants is None:
            raise RuntimeError("Power plants were not fetched before export")
        if args.export_plants_csv:
            plants_csv_path = Path(args.export_plants_csv)
        else:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            plants_csv_path = POSTERS_DIR / f"{slugify(args.country)}_plants_{timestamp}.csv"
        save_csv(raw_plants, plants_csv_path)
        print(f"Saved plants CSV: {plants_csv_path}")

    if args.export_substations_geojson is not None or args.export_substations_csv is not None:
        raw_substations = fetch_power_substations(
            country=args.country,
            boundary=boundary_wgs84,
            tile_size_km=args.tile_size_km,
            render_crs=args.crs,
    ...
    if args.export_substations_geojson is not None:
        if args.export_substations_geojson:
            substations_geojson_path = Path(args.export_substations_geojson)
        else:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            substations_geojson_path = POSTERS_DIR / f"{slugify(args.country)}_substations_{timestamp}.geojson"
        save_geojson(raw_substations, substations_geojson_path)
        print(f"Saved substations GeoJSON: {substations_geojson_path}")

    if args.export_substations_csv is not None:
        if args.export_substations_csv:
            substations_csv_path = Path(args.export_substations_csv)
        else:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            substations_csv_path = POSTERS_DIR / f"{slugify(args.country)}_substations_{timestamp}.csv"
        save_csv(raw_substations, substations_csv_path)
        print(f"Saved substations CSV: {substations_csv_path}")

    if args.export_turbines_geojson is not None or args.export_turbines_csv is not None:
        ...
        export_turbines = prepare_wind_turbines(raw_turbines)
        print(f"Wind turbines fetched: {len(export_turbines):,}")

    if args.export_turbines_geojson is not None:
        if args.export_turbines_geojson:
            turbines_geojson_path = Path(args.export_turbines_geojson)
        else:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            turbines_geojson_path = POSTERS_DIR / f"{slugify(args.country)}_wind_turbines_{timestamp}.geojson"
        save_geojson(export_turbines, turbines_geojson_path)
        print(f"Saved wind turbines GeoJSON: {turbines_geojson_path}")

    if args.export_turbines_csv is not None:
        if args.export_turbines_csv:
            turbines_csv_path = Path(args.export_turbines_csv)
        else:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            turbines_csv_path = POSTERS_DIR / f"{slugify(args.country)}_wind_turbines_{timestamp}.csv"
        save_csv(export_turbines, turbines_csv_path)
        print(f"Saved wind turbines CSV: {turbines_csv_path}")
